...
function handler(req, res) {
    ...
    async function detailUser() {
        ...
        const user_profile = await sequelize.query(`${searchUserProfile(req.user.id_user)}`, { type: QueryTypes.SELECT });

        const profile_permission = await sequelize.query(`${searchProfilePermission(req.user.id_user)}`, { type: QueryTypes.SELECT })
